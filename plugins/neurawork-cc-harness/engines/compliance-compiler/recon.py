"""Read-only install recon for the compliance-compiler skill.
...
a catalog has already been extracted.

Run:  python3 engines/compliance-compiler/recon.py
"""
...
def _find_existing_dir(root: Path) -> str | None:
    """A top-level dir that already looks like an installed compliance-compiler."""
    for child in sorted(root.iterdir()):
        if not child.is_dir() or child.name.startswith("."):
            continue
        if (child / "hooks" / "co-post-tooluse.py").exists() and \
           (child / "scripts" / "extract.py").exists():
            return child.name
    return None
...
def _catalog_built(root: Path, cdir: str | None) -> bool:
    if not cdir:
        return False
    catalog = root / cdir / "catalog"
    return catalog.is_dir() and any(catalog.glob("*.json"))
...
def main() -> int:
    ...
    catalog_built = _catalog_built(root, existing_dir)
    ...
    print(f"Catalog built: {catalog_built}")
    print(f"Timezone: {tz}")
