"""PreCompact hook — capture the transcript before auto-compaction discards it.

Same shape as cl-session-end.py but fires before Claude Code compacts a long
session. Requires a few real turns (MIN_TURNS_TO_FLUSH) and tolerates an empty
transcript_path (a known Claude Code edge case).
"""
...
LOG_TAG = "[cl-pre-compact]"


def effective_root() -> Path:
    """Where to write output: main checkout's ldir if in a worktree, else here."""
...
def main() -> None:
    ...
    env = {**child_env(), "LERNER_ROOT": str(root)}
    try:
